...
class TestDiscoveryPipeline(unittest.TestCase):
    ...
    def test_run_weekly_discovery(
        self, MockTMDB, MockRadarr, MockSonarr, MockJellyfin, MockGemini, MockDB
    ):
        # Setup mocks
        ...
        mock_jellyfin.get_recent_items.return_value = {
            "Items": [{"ExternalIds": {"Tmdb": "3"}}]
        }

        mock_tmdb = MockTMDB.return_value
        mock_tmdb.discover_new_releases.return_value = {
            "results": [{"id": 4, "title": "New Movie", "overview": "..."}]
        }
        mock_tmdb.discover_new_tv.return_value = {
            "results": [{"id": 5, "name": "New Show", "overview": "..."}]
        }
