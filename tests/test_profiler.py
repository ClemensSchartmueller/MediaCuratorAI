...
class TestProfiler(unittest.TestCase):
    ...
    def test_run(self, MockJellyfin, MockGemini, MockDB):
        ...
        mock_jellyfin_instance.get_watch_history.return_value = {
            'Items': [
                {'Name': 'Test Movie', 'Genres': ['Action'], 'Overview': 'An action movie.'}
            ]
        }
        
        mock_gemini_instance = MockGemini.return_value
        mock_gemini_instance.generate_taste_profile.return_value = "User likes Action movies."
        ...
        mock_db_instance.save_taste_profile.assert_called_once_with("User likes Action movies.")
