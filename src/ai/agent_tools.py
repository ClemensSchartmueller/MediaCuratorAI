...
def _execute_with_retry(fn, notify_fn, label, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            notify_fn(f"⚠️ Rate limit or API error during {label}. Retrying in {delay}s... (Attempt {attempt+1}/{max_retries})")
            time.sleep(delay)
            delay *= 2
...
def create_tools(tmdb, radarr, sonarr, notify_fn):
    ...
    def add_movie_to_library(title: str) -> str:
        ...
        def action():
            ...
            movies = [r for r in results.get('results', []) if r.get('media_type') == 'movie']
            if not movies:
    ...
    def add_series_to_library(title: str) -> str:
        ...
        def action():
            results = tmdb.search_multi(title)
            series = [r for r in results.get('results', []) if r.get('media_type') == 'tv']
            if not series:
    ...
    def discover_by_genre(genre_name: str, media_type: str) -> str:
        ...
        def action():
            ...
            for i in items:
                ...
                overview = i.get('overview', '')[:120] + "..."
                res.append(f"- **{name}** (Rating: {rating})\n  _{overview}_")
            return f"Top 5 popular {media_type}s in genre '{genre_name}':\n" + "\n".join(res)
            
        return _execute_with_retry(action, notify_fn, f"discovering {media_type} in genre '{genre_name}'")
