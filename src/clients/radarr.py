    ...
    def add_movie(self, tmdb_id, root_folder_path, quality_profile_id):
        ...
        try:
            ...
            if root_folder_path not in available_paths:
                print(
                    f"Warning: Configured root folder '{root_folder_path}' does not exist in Radarr. Falling back to '{available_paths[0]}'."
                )
                root_folder_path = available_paths[0]
        except Exception as e:
            print(
                f"Warning: Failed to fetch root folders from Radarr: {str(e)}. Using configured path: {root_folder_path}"
            )
        ...
        try:
            ...
            if quality_profile_id not in available_profile_ids:
                print(
                    f"Warning: Configured quality profile ID '{quality_profile_id}' does not exist in Radarr. Falling back to ID '{available_profile_ids[0]}'."
                )
                quality_profile_id = available_profile_ids[0]
        except Exception as e:
            print(
                f"Warning: Failed to fetch quality profiles from Radarr: {str(e)}. Using configured ID: {quality_profile_id}"
            )
